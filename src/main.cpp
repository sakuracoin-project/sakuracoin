...
#if defined(NDEBUG)
# error "Sakuracoin cannot be compiled without assertions."
#endif
...
CMedianFilter<int> cPeerBlockCounts(8, 0); // Amount of blocks that other nodes claim to have TODO
...
{
    return GetNextWorkRequired_V1(pindexLast, pblock); // TODO
}
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x47ff0d1bdfb5e1716a648be600c5f8e433799928172a89f9fcbac086619a5ad9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Dec. 19, 2013  Tokyo Gov. Inose announces resignation over money scandal";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 0x52b28510;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 0x000fc9f2;

        if (fTestNet)
        {
            block.nTime    = 1394862581;
            block.nNonce   = 78282;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x2db1a29d339a15d52b5ae43d70d5d77446011c5b275d3537d2f75cfc40b322dd"));
        block.print();
